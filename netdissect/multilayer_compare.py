import argparse, os, shutil, json, numpy
...
def parseargs():
    ...
    aa('--seg', choices=['netpqc'], default='netpqc')
    aa('--layers', nargs='+', default=[
        'features.conv1_1',
        'features.conv1_2',
        'features.conv2_1',
        'features.conv2_2',
    ...
    aa('--samplesize', type=int, default=1000)
...
def load_intersect_iou(dirname):
    def rf(s):
        return os.path.join(dirname, s)
    intersect_99 = runningstats.RunningVariance(state=numpy.load(
        rf('intersect_99.npz')))
    union_99 = runningstats.RunningVariance(state=numpy.load(
        rf('union_99.npz')))
    score = (intersect_99.mean() / union_99.mean()).clamp(0)
    return score

...
def load_proggan(domain):
    ...
    weights_filename = dict(
        ...
        celebhq='proggan_celebhq-620d161c.pth',
        car='/data/vision/torralba/ganprojects/placesgan/tracer/blue_firetruck/weights/lsun_cars.pt')[domain]

    #TODO: get this on gandissect
    if os.path.exists(weights_filename):
        return proggan.from_state_dict(torch.load(weights_filename))
...
g_datasets = {'car': {'train': '/data/vision/torralba/ganprojects/placesgan/datasets/minilsun/car/', 
                      'val': '/data/vision/torralba/ganprojects/placesgan/tracer/blue_firetruck/imgs'}}

def load_dataset(domain, split=None, full=False, download=True):
    if domain in g_datasets:
        dirname = g_datasets[domain][split]
        ...
        if split is None:
            split = 'val'
        dirname = 'dataset/imagenet/%s' % split
        return parallelfolder.ParallelImageFolders([dirname],
                classification=True,
                shuffle=True,
                transform=g_imagenet_transform)
    else:
        # Assume lsun dataset
        if split is None:
            split = 'train'
...
g_imagenet_transform = torchvision.transforms.Compose([
    torchvision.transforms.Resize(256),
    torchvision.transforms.CenterCrop(224),
    torchvision.transforms.ToTensor(),
    renormalize.NORMALIZER['imagenet_meanonly']])
